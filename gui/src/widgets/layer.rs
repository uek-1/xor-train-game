...
use egui::{Area, LayerId, Order, Pos2, Rect, RichText, Sense, Vec2, Widget};
use std::fmt::Write;

pub fn layer_ui(
    ui: &mut egui::Ui,
    layer: &mut puffpastry::Layer<f64>,
    radius: f32,
) -> egui::Response {
    let layer_info = format!(
        "Layer: Neurons: {}, Dimensions: {} x {}, Activation {:?}",
        layer.weights.len(),
        layer.weights[0].len(),
        layer.weights.len(),
        layer.activation
    ...
    let top_left =
        ui.min_rect().min + Vec2::new(-1.25 * radius, -4.0 * radius * layer.weights.len() as f32);
    let bottom_right = ui.min_rect().min
        + Vec2::new(
            1.25 * radius + 15.0,
            2.5 * radius * layer.weights.len() as f32,
        );
    ...
    ui.interact(ui.min_rect(), ui.next_auto_id(), Sense::hover())
...
pub fn layer(layer: &mut puffpastry::Layer<f64>, radius: f32) -> impl Widget + '_ {
