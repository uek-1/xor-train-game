use egui::{
    widgets::plot::{self, MarkerShape, Plot, PlotUi, Points, Text},
    Area, Layout, Rect, ScrollArea, Sense, Vec2,
};
use puffpastry::{self, Activation, Model};
...
impl LayerState {
    pub fn new() -> Self {
        LayerState {
            neuron_count_string: String::from(""),
...
pub struct LayerMenuState {
    ...
    layer_to_add: Option<(usize, puffpastry::Layer<f64>)>,
}

pub fn model<'a>(
    model: &'a mut Model<f64>,
    layer_state: &'a mut LayerState,
) -> impl egui::Widget + 'a {
...
pub fn model_ui(
    ...
    let return_response = ui
        .horizontal_centered(|ui| {
            create_layers_ui(ui, model, radius, layer_state, &mut layer_menu_state)
        })
        .response;

    match (
        layer_menu_state.layer_to_delete,
        layer_menu_state.last_layer_index,
    ) {
        (Some(num), Some(last)) if num != last => {
            model.layers.remove(num);
            ()
        }
        _ => (),
    }
...
fn create_layers_ui(
    ...
    radius: f32,
    layer_state: &mut LayerState,
    layer_menu_state: &mut LayerMenuState,
) {
    model
        .layers
        .iter_mut()
...
fn layer_context_menu(
    ...
    ui.menu_button("New", |ui| {
        ...
        if ui.button("Add").clicked() {
            ...
            layer_menu_state.layer_to_add = Some((
                layer_menu_state.index,
                puffpastry::Layer::from_size(
                    input_shape,
                    output_shape,
                    layer_state.activation.clone(),
                ),
            ));
            ui.close_menu();
        };
    ...
    if layer_menu_state.index != layer_menu_state.last_layer_index.unwrap()
