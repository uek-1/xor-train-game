use egui::{vec2, Color32, Frame, Painter, Pos2, Rect, RichText, Sense, Stroke, Vec2};
...
fn main() {
    ...
    let model: Model<f64> = Model::from_layers(
        vec![Layer::from_size(2, 2, Activation::Sigmoid)],
    ...
    pub fn get_evaluate_string(&self) -> String {
        ...
        for epoch in 0..train.len() {
            writeln!(
                ...
                "Prediction: XOR ({:?}) = {:?}",
                train[epoch],
                self.model.evaluate(&train[epoch])
            );
        }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        egui::SidePanel::right("Options Bar")
            .frame(Frame::default())
            .show(ctx, |ui| ui.add(widgets::control_panel(self)));
